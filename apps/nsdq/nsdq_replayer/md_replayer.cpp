...
int main(int argc, char* argv[])
{
    try
    {
        ...
        if(!std::filesystem::exists(configPath)){ std::cerr << "[Replayer] config.json not found\n"; return 1; }
        ...
        if(!std::filesystem::exists(filePath)){ std::cerr << "[Replayer] File does not exist: " << filePath << "\n"; return 1; }
    ...
    catch(const std::exception &e){ std::cerr << "[Replayer] Exception: " << e.what() << "\n"; return 1; }
    return 0;
}
